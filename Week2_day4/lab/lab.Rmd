...
How many of the corporation’s employees serve in committees
```{sql connection=db_conect}
SELECT
  --COUNT(employee_id)
...
SELECT 
  ...
  committees.name
FROM employees INNER JOIN employees_committees
  ON employees.id = employees_committees.employee_id
INNER JOIN committees
  ON employees_committees.committee_id = committees.id
...
Re-use the CASE() operator from question 6 above to group employees who are members of committees into salary_class 'low' or 'high'. Count the number of committee members in each class.
```{sql connection=db_conect, }
SELECT
  CASE
      WHEN employees.salary < 40000 THEN 'Low'
      WHEN employees.salary >= 40000 THEN 'High'
      ELSE 'Salary record is missing'
  END AS salary_class,
COUNT(employees.id) AS num_of_commitee_members
FROM employees INNER JOIN employees_committees
  ON employees.id = employees_committees.employee_id
...
Group committee members into the teams in which they work, counting the number of committee members in each team (including teams with no committee members). Order the list by the number of committee members, highest first.
```{sql connection=db_conect}
SELECT
  teams.name AS team_name,
  COUNT(employees.id) AS num_commitee_members
FROM employees INNER JOIN employees_committees
  ON employees.id = employees_committees.employee_id
RIGHT JOIN teams
  ON employees.team_id = teams.id

--FULL OUTER JOIN employees_committees 
  --ON employees.id = employees_committees.employee_id
GROUP BY teams.name
ORDER BY num_commitee_members DESC
```

----------

How many employees are not in a committee?
```{sql connection=db_conect}
SELECT 
  COUNT(*)
FROM employees LEFT JOIN employees_committees
  ON employees.id = employees_committees.employee_id
